...
- 컬럼: 시각(occurred_at, KST 표시), 사용자, 카드 라벨, 허용 여부, **출처 배지(카드/수동)**
- 필터: 사용자별 / 날짜 범위 / 허용·거부 / **void 포함 여부**
- 정렬 기본: occurred_at DESC, 페이지네이션 (예: 50건/페이지)
- **admin 전용 액션** (admin role만 노출):
  - 상단 "+ 탭 추가" 버튼 → 모달 (사용자, 시각, 메모) → `POST /api/logs`
  - 행 액션 — 수정 (manual만 시각 편집, card는 메모만), void/복구 (모든 source)
  - voided 행은 strikethrough + 회색 처리

### `/stats` — 출퇴근 통계
- 출/퇴근 해석 규칙은 [[../domain/access-log#출퇴근-해석]] 따름
- **일별 뷰**: 사용자 선택 → 한 달치 출근/퇴근 시각 + 근무시간 표
- **월별 뷰**: 출근일수 / 평균 출근·퇴근 시각 / 평균 근무시간
- 처리는 **앱 레이어 (이 페이지 또는 백엔드)** — DB 뷰 안 만듦
- 일별 행에 "탭 보정" 링크 → `/logs?date=...&user_id=...` 필터 이동 (수동 입력 흐름은 `/logs`로 일원화)
